...
mysql_servers = (
	{ address = "mariadb-master1"; port = 3306; hostgroup = 10; max_connections = 2000; weight = 1000; max_replication_lag = 0; use_ssl = 0; max_latency_ms = 500; comment = "master1-primary-writer"; },
	{ address = "mariadb-master2"; port = 3306; hostgroup = 15; max_connections = 500; weight = 1000; max_replication_lag = 0; use_ssl = 0; max_latency_ms = 500; comment = "master2-backup-standby"; },
...
mysql_users = (
	{ username = "root"; password = "${MYSQL_ROOT_PASSWORD}"; default_hostgroup = 10; transaction_persistent = 1; max_connections = 4000; fast_forward = 0; active = 1; },
	{ username = "${MYSQL_REPL_USER:-repl}"; password = "${MYSQL_REPL_PASSWORD}"; default_hostgroup = 10; max_connections = 500; fast_forward = 1; }
);

mysql_query_rules = (
	{ rule_id = 1; active = 1; match_digest = "^CREATE.*|^ALTER.*|^DROP.*|^TRUNCATE.*|^RENAME.*"; destination_hostgroup = 10; apply = 1; comment = "DDL to master1"; },
	{ rule_id = 2; active = 1; match_digest = "^SET.*|^SHOW.*|^USE.*|^DESC.*|^DESCRIBE.*|^EXPLAIN.*"; destination_hostgroup = 10; apply = 1; comment = "Session commands to master1"; },
	{ rule_id = 3; active = 1; match_digest = "^BEGIN.*|^START TRANSACTION.*|^COMMIT.*|^ROLLBACK.*"; destination_hostgroup = 10; apply = 1; comment = "Transactions to master1"; },
	{ rule_id = 4; active = 1; match_digest = "^SELECT.*FOR UPDATE.*|^SELECT.*LOCK IN SHARE MODE.*"; destination_hostgroup = 10; apply = 1; comment = "Locking reads to master1"; },
	{ rule_id = 5; active = 1; match_digest = "^INSERT.*|^UPDATE.*|^DELETE.*|^REPLACE.*|^LOAD DATA.*"; destination_hostgroup = 10; apply = 1; comment = "DML to master1"; },
